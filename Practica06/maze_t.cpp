...
{ 
  ...
  return (i > 0 && i <= matrix_.get_m() && j > 0 && j <= matrix_.get_n() && 
...
maze_t::solve_(const int i, const int j)
{
  ...
  // guardamos el camino hacia la salida en forma dentro de una cola
  for (int k = 0; k <= 3; ++k) {
    if (is_ok_(i + i_d[k], j + j_d[k])) {
      if (solve_(i + i_d[k], j + j_d[k])) {
        ...
        pair_t<int> marker(j,i);
        solution_.push(marker);      //cola  
        return true;
      }
    }
  }
...
// imprime la solucion del laberinto
void maze_t::WriteSolution() const {
  solution_.write();
}
